...
public class KeyControl extends KeyAdapter implements Runnable {
	...
	boolean KeySpace = false; // bullet발사.
	boolean KeyCtrl = false; // item.
	boolean KeyZ = false; // skill.

	Player player;
	int cnt; // bullet 발사의 주기를 낮추기 위한 카운트.
	...
	public KeyControl(Player player, GameObject GameObject) {
		...
		shootingsound = new Music("src/main/SOURCE/missileSound.wav");
	...
	public void keyProcess() {
		...
		if (KeySpace == true) { // bullet 발사.
			// bullet 발사시, bullet 객체 만들고 bullet들을 모아두는 리스트에 추가.

			if (cnt % 8 == 0) {
				shootingsound.playOnetime(2);// 발사때마다 효과음 발생.

				if (this.player.user.currentplane.name == "F4K") {
		...
		if (KeyCtrl == true) { // item.
			if (this.player.user.currentitem != null) {
				this.player.HP += this.player.user.currentitem.HPPlus;
				this.player.user.currentitem = null;
				if (this.player.HP >= this.player.user.currentplane.HP) {
					this.player.HP = this.player.user.currentplane.HP;
				}
			}
		...
		if (KeyZ == true) { // skill.
			if (this.GameObject.checkSkill == true) {
				if (this.player.user.currentplane.name == "F15K") {
				...
				else if (this.player.user.currentplane.name == "F35K") {

//					
//					// enemy bullet 다 없애기. ( 시간차로 인해서 3번 반복문으로 확실히 다 없애준다. )
...
//					
	...
	public void PlayerProcess() {
		// 플레이어와 enemy plane끼리의 충돌시 처리.
		for (int j = 0; j < this.GameObject.Enemys.size(); j++) {

			if (com.won.GameObject.Crash(this.player.posX, this.player.posY, this.GameObject.Enemys.get(j).posX,
					this.GameObject.Enemys.get(j).posY, this.player.width, this.player.height,
					this.GameObject.Enemys.get(j).width, this.GameObject.Enemys.get(j).height)) {
	...
	// 플레이어가 쏜 bullet에 대한 처리.
	public void PlayerBullet() {
		// 플레이어가 쏜 bullet에 enemy이 맞았을 때에 대한 처리.
		for (int i = 0; i < this.GameObject.Bullets.size(); i++) {
			Bullet bullet = this.GameObject.Bullets.get(i);

			// 플레이어 bullet이 일반 enemy plane에 맞을때 처리.
			for (int j = 0; j < this.GameObject.Enemys.size(); j++) {
				if (GameObject.Crash(bullet.posX, bullet.posY, this.GameObject.Enemys.get(j).posX,
						this.GameObject.Enemys.get(j).posY, bullet.width, bullet.height,
						this.GameObject.Enemys.get(j).width, this.GameObject.Enemys.get(j).height) ) {
					...
					if (this.GameObject.Enemys.get(j).HP == 0) {
						...
						Effect effect = new Effect(this.GameObject.Enemys.get(j).posX,
								this.GameObject.Enemys.get(j).posY);
			...
			// 플레이어 bullet이 boss가 맞을때 처리.
			for (int k = 0; k < this.GameObject.Bosses.size(); k++) {
				if (GameObject.Crash(bullet.posX, bullet.posY, this.GameObject.Bosses.get(k).posX,
						this.GameObject.Bosses.get(k).posY, bullet.width, bullet.height,
						this.GameObject.Bosses.get(k).width, this.GameObject.Bosses.get(k).height)) {
					...
					if (this.GameObject.Bosses.get(k).HP == 0) {
						...
						BEffect beffect = new BEffect(this.GameObject.Bosses.get(k).posX,
								this.GameObject.Bosses.get(k).posY);
	...
	public void Enemybullet() {
		...
		// enemy이 쏜 bullet이 플레이어에 맞았을때 처리.
		for (int i = 0; i < this.GameObject.Ebullets.size(); i++) {
			if (GameObject.Crash(this.GameObject.Ebullets.get(i).posX, this.GameObject.Ebullets.get(i).posY,
					this.player.posX, this.player.posY, this.GameObject.Ebullets.get(i).width,
					this.GameObject.Ebullets.get(i).height, this.player.width, this.player.height)) {
